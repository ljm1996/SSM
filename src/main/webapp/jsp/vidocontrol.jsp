...
<head>
  ...
  <script>
           ...
           var table =  $('#table_id_example').DataTable({
			  ...
			  //使用ajax异步请求
                ...
                columnDefs: [
                {
    				...
    				"targets": 2,
      				"render": function(data, type, row, meta) {
		            	 return '<button type="button" class="btn btn-red"  onclick=\'deletbykey("'+row.id+'")\'>删除</button>';
		        	}
    			}     
           ],
            });
	...
	function deletbykey(id)
	{    
	     alert(id);
	     var flag = confirm("确定删除吗?");
	     if(flag){
		 $.ajax({
	        url: '${pageContext.request.contextPath}/video/deletbykey',
	        method:'post',
	        //在后台接受id这个参数
	        data: {
	        	id:id
